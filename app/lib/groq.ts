...
Generate the next follow-up question that goes deeper.`;
  ...
  try {
    ...
    return {
      type: "prompt",
      text: "What is one small thing you could do differently tomorrow that is actually within your power?",
    };
  }
}
...
After: mood=${postSliders.mood}%, mental_noise=${postSliders.mentalNoise}%, focus=${postSliders.focus}%, energy=${postSliders.energy}%`;
  ...
  try {
    ...
    return {
      type: "prompt",
      text: "You showed up and put words to what you were carrying.",
    };
  }
}
